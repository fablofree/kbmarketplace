<div class="kb-panel outer-border">
    ...
    <div id="kb-product-type-form" class='kb-panel-body'>
        ...
        <form id="kb-product-type-selection" action="{$form_submit_url nofilter}" method="post">  {*Variable contains HTML/CSS/JSON, escape not required *}
           ...
           <div class="kb-row kb-inpoption">
                <input class="" type="hidden" id="label_for_pack_product" name="kb_product_type" value="{$type_pack|intval}" /> <label  for="label_for_pack_product">{*l s='Pack of existing products' mod='kbmarketplace'*}</label>    
            </div>
            <div class="kb-row kb-inpoption">
                <input class="" type="hidden" id="label_for_virtual_product" name="kb_product_type" value="{$type_virtual|intval}" /> <label  for="label_for_virtual_product">{*l s='Virtual product (services, booking, downloadable products, etc.)' mod='kbmarketplace'*}</label>    
            </div>
