...
require_once(_PS_MODULE_DIR_.'whatsappchat/classes/WhatsappChatBlock.php');
...
class KbmarketplaceSellerfrontModuleFrontController extends KbmarketplaceFrontCoreModuleFrontController
{
    ...
    {
        parent::setMedia();
        $this->addCSS(_MODULE_DIR_ . 'kbmarketplace/views/css/front/custom.css');
    ...
    public function renderSellerList()
    {
        ...
        */
        ...
        if ($total > 0) {
            ...
            //associate whatsapp chat to seller
            $whatsapp = new WhatsappChatBlock();
            
            $sellers_array = array();
            foreach ($sellers as $seller) {
                
                $results = $whatsapp->getWhatsappChatByHookAndBySellerId($seller['id_seller']);
                if(!empty($results) && is_array($results)){
                    foreach ($results as $result){
                        
                        $text = $result['def_message'];
                        $text .= ' '.$whatsapp->getProtocolUrl().$_SERVER['HTTP_HOST'].$_SERVER['REQUEST_URI'];
                        $url = $whatsapp->getWhatsappUrl($result['mobile_phone'], $text, $result['chat_group']);
                        $seller['url_whatsapp'] = $url;
                        $seller['color'] = $result["color"];
                    }
                }
                $sellers_array[] = $seller;
            }

            $this->context->smarty->assign('sellers', $sellers_array);
            ...
            if ($seller->isSeller()) {
                ...
                */
                ...
                $sellers_array = array();
                    ...
                    $results = $whatsapp->getWhatsappChatByHookAndBySellerId($seller_info['id_seller']);
                    ...
                    if(!empty($results) && is_array($results)){
                        foreach ($results as $result){
                            ...
                            $text .= ' '.$whatsapp->getProtocolUrl().$_SERVER['HTTP_HOST'].$_SERVER['REQUEST_URI'];
                            $url = $whatsapp->getWhatsappUrl($result['mobile_phone'], $text, $result['chat_group']);
                            $seller_info['url_whatsapp'] = $url;
                            $seller_info['color'] = $result["color"];
                        }
                    }
                    $sellers_array[] = $seller;
